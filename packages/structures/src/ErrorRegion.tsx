...
interface ErrorRegionRootProps extends Omit<BaseProps, "children"> {
	 ...
	 *
	 * By default this is used as a name of the region navigational landmark (deprecated behavior that will be removed in a future release). `aria-label` or `aria-labelledby` prop should be provided to explicitly label the region instead.
	 */
 ...
 * and should be labelled using `aria-label` or `aria-labelledby`. The component should not be rendered conditionally, instead use the `items` prop to control the visibility.
 *
 * Changes to the `label` prop will be announced communicated using a [live region](https://developer.mozilla.org/en-US/docs/Web/Accessibility/ARIA/Guides/Live_regions).
 *
 *
 * Example:
 * ```tsx
