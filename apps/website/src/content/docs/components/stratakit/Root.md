...
The **Root** component must wrap your application to load StrataKit styles and fonts. It applies StrataKit theme for MUI enabling you to use [MUI components](https://mui.com/material-ui/all-components/) with StrataKit's visual language.
...
Use the [`rootNode`](/reference/mui/Root#Root.Root.rootNode) prop to customize the node to which this **Root** component is attached. This needs to be set when the **Root** is rendered within [shadow DOM](https://developer.mozilla.org/en-US/docs/Web/API/Web_components/Using_shadow_DOM) or a [popup window](https://developer.mozilla.org/en-US/docs/Web/API/Window/open#description).
